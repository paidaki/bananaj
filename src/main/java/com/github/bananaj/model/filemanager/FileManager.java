...
public class FileManager {
    ...
    public FileManagerFolder getFileManagerFolder(int id) throws Exception {
        ...
        JSONObject jsonFileManagerFolder = new JSONObject(
                getConnection().do_Get(new URL(getConnection().getFilemanagerfolderendpoint() + "/" + id), connection.getApikey()));
    ...
    public FileManagerFolder createFileManagerFolder(String name) throws Exception {
        ...
        JSONObject jsonFileManagerFolder = new JSONObject(
                getConnection().do_Post(new URL(getConnection().getFilemanagerfolderendpoint()), newFolder.toString(), getConnection().getApikey()));
        return new FileManagerFolder(getConnection(), jsonFileManagerFolder);
    }
    ...
    public FileManagerFile getFileManagerFile(int id) throws Exception {
        // parse response
        JSONObject jsonFileManagerFile = new JSONObject(
                getConnection().do_Get(new URL(getConnection().getFilesendpoint() + "/" + id), getConnection().getApikey()));
