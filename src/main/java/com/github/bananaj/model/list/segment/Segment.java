...
public class Segment {
    ...
    public void addMember(Member member) throws Exception {
        ...
        connection.do_Post(new URL(connection.getListendpoint() + "/" + this.getListId() + "/segments/" + this.getId() + "/members"), jsonObj.toString(),
    ...
    public ArrayList<Member> getMembers(int count, int offset) throws Exception {
        ...
        if (count != 0) {
            list = new JSONObject(connection.do_Get(
                    new URL(connection.getListendpoint() + "/" + this.getListId() + "/segments/" + this.getId() + "/members?count=" + count + "&offset=" + offset),
                    connection.getApikey()));
        }
        else {
            list = new JSONObject(connection.do_Get(
                    new URL(connection.getListendpoint() + "/" + this.getListId() + "/segments/" + this.getId() + "/members?count=" + this.getMemberCount() + "&offset=" + offset),
                    connection.getApikey()));
        }
    ...
    public void removeMember(Member member) throws Exception {
        ...
        connection.do_Delete(new URL(connection.getListendpoint() + "/" + this.getListId() + "/segments/" + this.getId() + "/members/" + member.getId()),
                             connection.getApikey());
    }
    ...
    public void update(Segment updatedSegment) throws Exception {
        ...
        connection.do_Patch(new URL(connection.getListendpoint() + "/" + this.getListId() + "/segments/" + this.getId()),
    ...
    public void updateMembers(String[] emails) throws MalformedURLException, TransportException, URISyntaxException {
        ...
        String results = connection.do_Patch(new URL(connection.getListendpoint() + "/" + getListId() + "/segments/" + getId()), json.toString(),
                                             connection.getApikey());
    ...
    public void update() throws MalformedURLException, TransportException, URISyntaxException {
        ...
        String results = connection.do_Patch(new URL(connection.getListendpoint() + "/" + getListId() + "/segments/" + getId()),
                                             getJSONRepresentation().toString(), connection.getApikey());
        parse(connection, new JSONObject(results));  // update this object with current data
    }
